	...
	<div class="main">
		...
		<center>
			...
			if ($_SERVER["REQUEST_METHOD"] == "POST") {
				...
				if ($result->num_rows > 0) {
					...
					while ($row = $result->fetch_assoc()) {
						echo "<p style= background-color:red;font-weight:bold;font-size:20px>Theater ID: " . $row["Theater_id"] . " Theater: " . $row["Name"] . " " . $row["Address"] . " " . $row["City"] . " " . $row["StateL"] . " " . $row["Zip_code"] . "</p><br>";
					}
				} else {
					echo "<p style= background-color:red;font-weight:bold;font-size:20px>0 results </p>"; //if no results are found it prints 0 results
				}
			...
			<form method="post" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" class="location">>
				...
				<button type="submit" name="submit" id="cont" onclick="location.href='movie.php';" class="continue">Continue</button>
